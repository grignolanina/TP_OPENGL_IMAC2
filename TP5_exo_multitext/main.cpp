...
int main()
{
    ...
    ctx.update = [&]() {
        ...
        for(int i = 0; i<32; i++){
            ...
            //texture lune bind multitext
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, 0); //debind text terre il y a rien
            glActiveTexture(GL_TEXTURE1);
            glBindTexture(GL_TEXTURE_2D, textureMoon); //bind text lune à la place nuage
            glUniform1i(uText1, 0);
